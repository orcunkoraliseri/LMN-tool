...
1. User views filtered neighbourhoods on `layer0_NUs_selection.html`
2. User clicks on a neighbourhood's **EUI value** in the Energy column
3. User is redirected to `layer1_energy_breakdown.html?neighbourhood=RC1` (or selected code)
...
#### [NEW] [layer1_energy_breakdown.html](file:///Users/orcunkoraliseri/Desktop/Interface/layer1_energy_breakdown.html)
...
euiCell.addEventListener('click', () => {
  window.location.href = `layer1_energy_breakdown.html?neighbourhood=${neighbourhood.code}`;
});
```
